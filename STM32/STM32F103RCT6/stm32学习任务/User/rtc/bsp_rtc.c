...
void RTC_CheckAndConfig(struct rtc_time *tm)
{
	int num = 0;
	  ...
	  则需重新配置时间并询问用户调整时间*/
	printf("\n\n\n\n%s %04x \n\n\n\n",__FUNCTION__, BKP_ReadBackupRegister(BKP_DR1));
...
{
	  u32 Tmp_YY = 0xff, Tmp_MM = 5, Tmp_DD = 14, Tmp_HH = 10, Tmp_MI = 35, Tmp_SS = 0;
...
extern struct rtc_time rtc_demo_time;
...
void Time_Display(uint32_t TimeVar,struct rtc_time* tm)
{
	  ...
	  printf(" UNIX时间戳 = %d 当前时间为: %d年(%s年) %d月 %d日 (星期%s)  %0.2d:%0.2d:%0.2d\r",TimeVar,
	                    ...
	                    tm->tm_min, tm->tm_sec);
	  jieshu2();
	  printf("n0.val=%0.2d",tm->tm_hour);
	  jieshu2();
	  
	  printf("n1.val=%0.2d",tm->tm_min);
	  jieshu2();
	  printf("n2.val=%0.2d",tm->tm_sec);
	  jieshu2();
}
